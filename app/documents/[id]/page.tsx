...
export default function DocumentAnalysisPage() {
  ...
  // Active view tab for right panel: "clauses" | "findings" | "summary" | "qa" | "fairness" | "options"
  const [activeAnalysisTab, setActiveAnalysisTab] = React.useState<"clauses" | "findings" | "summary" | "qa" | "fairness" | "options">("clauses");
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          <div className="border-b border-slate-200 bg-white px-4 pt-2 shrink-0 flex items-center justify-between">
            <div className="flex gap-1" role="tablist" aria-label="Analysis Tabs">
              ...
              <button
                id="tab-options"
                role="tab"
                aria-selected={activeAnalysisTab === "options"}
                aria-controls="panel-options"
                onClick={() => setActiveAnalysisTab("options")}
                className={`px-3.5 py-2 text-xs font-semibold rounded-t-lg transition-colors border-b-2 flex items-center gap-1.5 ${
                  activeAnalysisTab === "options"
                    ? "border-primary-800 text-primary-950 bg-primary-50/50"
                    : "border-transparent text-slate-600 hover:text-slate-900"
                }`}
              >
                ...
                <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">
                  Actionable
                </span>
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
            ...
            {/* ========================================================
                TAB 6: USER OPTIONS, ACTIONABLE CHECKLISTS & NEXT STEPS
               ======================================================== */}
            {activeAnalysisTab === "options" && (
              <div
                role="tabpanel"
              ...
              >
                ...
                <div className="rounded-xl border border-primary-100 bg-gradient-to-r from-primary-900 to-primary-950 p-5 text-white shadow-sm">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold tracking-wide uppercase bg-emerald-400 text-slate-950">
                      Problem Alignment
                    </span>
                  ...
                  <h3 className="text-base font-bold text-white tracking-tight">
                    Understanding Your Options & Potential Next Steps
                  </h3>
                  <p className="text-xs text-primary-200 mt-1 max-w-xl leading-relaxed">
                    Based on your analyzed agreement, here are your strategic options, evaluated residual risks, and an interactive preparation checklist.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                    Strategic Pathways (Your Options)
                  </h4>
                  <div className="grid grid-cols-1 gap-3">
                    {/* Option 1: Accept with Awareness */}
                    <Card className="border-slate-200 bg-white shadow-xs">
                      <CardHeader className="p-4 pb-2 bg-slate-50/60 border-b border-slate-100">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-slate-900 flex items-center gap-1.5">
                            <span className="w-5 h-5 rounded-full bg-slate-200 text-slate-800 text-[11px] font-bold flex items-center justify-center">1</span>
                            Option A: Execute / Sign As-Is (Accept Terms)
                          </span>
                      ...
                      <CardContent className="p-4 space-y-2 text-xs">
                        <p className="text-slate-600 leading-relaxed">
                          If commercial urgency requires signing immediately without negotiation, be mindful of the following key binding covenants:
                        </p>
                        <ul className="space-y-1 text-slate-700 text-[11px]">
                          <li className="flex items-start gap-1.5">
                            <CheckCircle2 className="w-3.5 h-3.5 text-slate-400 shrink-0 mt-0.5" />
                            <span><strong>Post-Termination Obligations:</strong> Ensure you diarize notice windows ({currentDoc.summary?.duration || "Standard term"}).</span>
                          </li>
                          <li className="flex items-start gap-1.5">
                            <CheckCircle2 className="w-3.5 h-3.5 text-slate-400 shrink-0 mt-0.5" />
                            <span><strong>Risk Exposure:</strong> High-risk indemnity or IP covenants remain enforceable without modification.</span>
                          </li>
                        </ul>
                      </CardContent>
                    </Card>

                    {/* Option 2: Request Targeted Amendments */}
                    <Card className="border-primary-200 bg-primary-50/20 shadow-xs">
                      <CardHeader className="p-4 pb-2 bg-primary-50/50 border-b border-primary-100">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-primary-950 flex items-center gap-1.5">
                            <span className="w-5 h-5 rounded-full bg-primary-900 text-white text-[11px] font-bold flex items-center justify-center">2</span>
                            Option B: Propose Balanced Amendments (Recommended)
                          </span>
                      ...
                      <CardContent className="p-4 space-y-3 text-xs">
                        <p className="text-slate-700 leading-relaxed">
                          Push back on unilateral clauses using LexiGuide&apos;s AI-generated counter-proposals and tactical talking points.
                        </p>
                        <div className="flex items-center gap-2">
                          <Button
                            size="sm"
                            onClick={() => setActiveAnalysisTab("fairness")}
                            className="text-xs flex items-center gap-1"
                          >
                            <span>View Redline Counter-Proposals</span>
                    ...
                    {/* Option 3: Escalate to an Attorney */}
                    <Card className="border-amber-200 bg-amber-50/20 shadow-xs">
                      <CardHeader className="p-4 pb-2 bg-amber-50/50 border-b border-amber-100">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-amber-950 flex items-center gap-1.5">
                            ...
                            Option C: Escalate to a Licensed Attorney
                          </span>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-100 text-amber-900">
                            Complex / High-Stakes Path
                          </span>
                        </div>
                      </CardHeader>
                      <CardContent className="p-4 space-y-3 text-xs">
                        <p className="text-slate-700 leading-relaxed">
                          For high-stakes agreements, seek formal legal counsel. LexiGuide has pre-compiled an Attorney Consultation Brief so you don&apos;t waste billable hours.
                        </p>
                        <Link href={`/documents/${currentDoc.id}/consultation`}>
                          <Button size="sm" variant="outline" className="text-xs flex items-center gap-1 border-amber-300 hover:bg-amber-100 text-amber-900">
                            <Briefcase className="w-3.5 h-3.5" />
                            <span>Open Legal Consultation Brief</span>
                ...
                <Card className="border-slate-200 bg-white shadow-xs">
                  ...
                  <CardContent className="p-4 space-y-2.5 text-xs">
                    {[
                      { id: "step-1", text: "Review flagged high-attention provisions in the Structured Clauses tab." },
                      { id: "step-2", text: "Evaluate missing standard terms (cure periods, liability caps) in Fairness tab." },
                      { id: "step-3", text: "Copy proposed compromise wording for redline response to other party." },
                      { id: "step-4", text: "Confirm governing law jurisdiction and dispute resolution location." },
                      { id: "step-5", text: "Export the Consultation Brief (.md or PDF) if escalating to an attorney." },
                    ].map((item) => (
                      <label
                        key={item.id}
                      ...
                      >
                        <input
                          type="checkbox"
                          className="mt-0.5 h-4 w-4 rounded border-slate-300 text-primary-800 focus:ring-primary-600"
                        />
